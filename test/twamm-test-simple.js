  ...
  describe("Basic AMM", function () {
    describe("Providing Liquidity", function () {
      it("can't provide initial liquidity twice", async function () {
        const amount = 10000;
        await token0.approve(twamm.address, amount); //owner calls it
        await token1.approve(twamm.address, amount);
        await expect(
      ...
      it("(ETH)can't provide initial liquidity twice(ETH)", async function () {
        const amount = 10000;
        await WETH.approve(twamm.address, amount);
        await token.approve(twamm.address, amount);
        await expect(
      ...
      it("LP token value is constant after mint", async function () {
        let totalSupply = await twamm.obtainTotalSupply(
          token0.address,
          token1.address
        );
        ...
        await token0.approve(twamm.address, newLPTokens * initialToken0PerLP); //owner calls it
        await token1.approve(twamm.address, newLPTokens * initialToken1PerLP);

        await twamm.addLiquidity(
          token0.address,
          token1.address,
          newLPTokens,
          newLPTokens * initialToken0PerLP,
          newLPTokens * initialToken1PerLP,
          timeStamp + 100000
        );

        totalSupply = await twamm.obtainTotalSupply(
          token0.address,
          token1.address
        );
      ...
      it("(ETH)LP token value is constant after mint(ETH)", async function () {
        ...
        const newLPTokens = 10000;
        await WETH.approve(twamm.address, newLPTokens * initialETHPerLP); //owner calls it
        await token.approve(twamm.address, newLPTokens * initialTokenPerLP);

        await twamm.addLiquidityETH(
          token.address,
          newLPTokens,
          newLPTokens * initialTokenPerLP,
          newLPTokens * initialETHPerLP,
          timeStamp + 100000,
    ...
    describe("Instant Swaps", function () {
      it("instant swap expected amount", async function () {
        ...
        await token0.approve(twamm.address, amountIn); //owner calls it
      ...
      it("(ETH)instant swap expected amount(ETH)", async function () {
        ...
        //adjust for LP fee of 0.3%
        const expectedOutput = expectedOutBeforeFees.mul(1000 - 3).div(1000);
        await token.approve(twamm.address, amountTokenIn); //owner calls it
      ...
      it("(ETH)instant swap expected amount(ETH)", async function () {
        ...
        //adjust for LP fee of 0.3%
        const expectedOutput = expectedOutBeforeFees.mul(1000 - 3).div(1000);
        await WETH.approve(twamm.address, amountETHIn); //owner calls it
  ...
  describe("TWAMM Functionality ", function () {
    describe("Long Term Swaps", function () {
      it("single sided long term order behaves like normal swap", async function () {
        ...
        token0.connect(addr0).approve(twamm.address, amountIn);
        await twamm
          .connect(addr0)
      ...
      it("(ETH) single sided long term order behaves like normal swap (ETH)", async function () {
        ...
        const expectedOutput = expectedOutBeforeFees.mul(1000 - 3).div(1000);
        //trigger long term order
        token.connect(addr0).approve(twamm.address, amountTokenIn);
      ...
      it("(ETH) single sided long term order behaves like normal swap (ETH)", async function () {
        ...
        //trigger long term order
        WETH.connect(addr0).approve(twamm.address, amountETHIn);
        await twamm
          .connect(addr0)
      ...
      it("orders in both pools work as expected", async function () {
        ...
        //trigger long term order
        await token0.connect(addr0).approve(twamm.address, amountIn);
      ...
      it("(ETH) orders in both pools work as expected (ETH)", async function () {
        ...
        await token.connect(addr0).approve(twamm.address, amountIn);
        await WETH.connect(addr1).approve(twamm.address, amountIn);
      ...
      it("long term swap amounts are consistent with twamm formula", async function () {
        ...
        await token0.connect(addr0).approve(twamm.address, token0In);
        await token1.connect(addr1).approve(twamm.address, token1In);
      ...
      it("(ETH)long term swap amounts are consistent with twamm formula(ETH)", async function () {
        ...
        await token.connect(addr0).approve(twamm.address, tokenIn);
        await WETH.connect(addr1).approve(twamm.address, ethIn);
    ...
    describe("Cancelling Orders", function () {
      ...
      it("(ETH)order can be cancelled(ETH)", async function () {
        ...
        await token.connect(addr0).approve(twamm.address, amountTokenIn);
        await token.transfer(addr0.address, amountTokenIn);
        await token.connect(addr0).approve(twamm.address, amountTokenIn);
    ...
    it("(ETH)order can be cancelled(ETH)", async function () {
      ...
      await WETH.connect(addr0).approve(twamm.address, amountETHIn);
