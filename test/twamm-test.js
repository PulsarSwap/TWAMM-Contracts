    ...
    describe("Basic AMM", function () {
        ...
        describe("Providing Liquidity", function () {
            ...
            it("(ETH)can't provide initial liquidity twice(ETH)", async function () {
                ...
                const amount = 10000;
                await expect(
                    twamm.addInitialLiquidityETH(tokenB.address, amount, amount, timeStamp+100000, {value:amount})
                ).to.be.revertedWith('Liquidity Has Already Been Provided, Need To Call provideLiquidity');
            });
            ...
            it("(ETH)LP token value is constant after mint(ETH)", async function () {
                ...
                await twamm.addLiquidityETH(tokenB.address, newLPTokens, timeStamp+100000, {value:newLPTokens});
        ...
        describe("Removing Liquidity", function () {
            ...
            it("(ETH)LP token value is constant after removing(ETH)", async function () {
                ...
                const liquidityToRemove = initialLiquidityProvided / 2;
                await twamm.withdrawLiquidityETH(tokenB.address, liquidityToRemove, timeStamp+100000);
            ...
            it("(ETH)can't remove more than available liquidity(ETH)", async function () {
                ...
                await expect(
                    twamm.withdrawLiquidityETH(tokenB.address, liquidityToRemove, timeStamp+100000)
                ).to.be.revertedWith('Not Enough Lp Tokens Available');                
            });
    ...
    describe("TWAMM Functionality ", function () {
        ...
        describe("Long term swaps", function () {
            ...
            it("(ETH) Orders in both pools work as expected (ETH)", async function () {
                ...
                //trigger long term orders
                await twamm.connect(addr1).longTermSwapETHToToken(tokenB.address, amountIn, 2, timeStamp+100000, {value: amountIn});
                await twamm.connect(addr2).longTermSwapTokenToETH(tokenB.address, amountIn, 2, timeStamp+100000);
                ...
                //withdraw proceeds 
                await twamm.connect(addr1).withdrawProceedsFromTermSwapETHToToken(tokenB.address, 0, timeStamp+100000);
                await twamm.connect(addr2).withdrawProceedsFromTermSwapTokenToETH(tokenB.address, 1, timeStamp+100000);
            ...
            it("(ETH)Swap amounts are consistent with twamm formula(ETH)", async function () {
                ...
                //trigger long term orders
                await twamm.connect(addr1).longTermSwapETHToToken(tokenB.address, tokenAIn, 2, timeStamp+100000, {value: tokenAIn});
                await twamm.connect(addr2).longTermSwapTokenToETH(tokenB.address, tokenBIn, 2, timeStamp+100000);
                ...
                expect(finalAReserveActual.toNumber()).to.be.closeTo(finalAReserveExpected, finalAReserveExpected/70);
                expect(finalBReserveActual.toNumber()).to.be.closeTo(finalBReserveExpected, finalBReserveExpected/70);

                expect(amountABought.toNumber()).to.be.closeTo(tokenAOut, tokenAOut/70);
                expect(amountBBought.toNumber()).to.be.closeTo(tokenBOut, tokenBOut/70);
            });
            ...
            it("(ETH)Multiple orders in both pools work as expected(ETH)", async function () {
                ...
                //trigger long term orders
                await twamm.connect(addr1).longTermSwapETHToToken(tokenB.address, amountIn/2, 2, timeStamp+100000, {value: amountIn/2});//, { 
                await twamm.connect(addr2).longTermSwapTokenToETH(tokenB.address, amountIn/2, 3, timeStamp+100000); 
                await twamm.connect(addr1).longTermSwapETHToToken(tokenB.address, amountIn/2, 4, timeStamp+100000, {value: amountIn/2});
                await twamm.connect(addr2).longTermSwapTokenToETH(tokenB.address, amountIn/2, 5, timeStamp+100000);
                ...
                await twamm.connect(addr1).withdrawProceedsFromTermSwapETHToToken(tokenB.address, 0, timeStamp+100000);
                await twamm.connect(addr2).withdrawProceedsFromTermSwapTokenToETH(tokenB.address, 1, timeStamp+100000);
                await twamm.connect(addr1).withdrawProceedsFromTermSwapETHToToken(tokenB.address, 2, timeStamp+100000);
                await twamm.connect(addr2).withdrawProceedsFromTermSwapTokenToETH(tokenB.address, 3, timeStamp+100000);
        ...
        it("(ETH)Normal swap works as expected while long term orders are active(ETH)", async function () {
            ...
            //trigger long term orders
            await twamm.connect(addr1).longTermSwapETHToToken(tokenB.address, amountIn, 10, timeStamp+100000, {value:amountIn});
            await twamm.connect(addr2).longTermSwapTokenToETH(tokenB.address, amountIn, 10, timeStamp+100000);
        ...
        describe("Cancelling orders", function () { 
            ...
            it("(ETH)Order can be cancelled(ETH)", async function () {
                ...
                //move blocks forward, and execute virtual orders
                await mineBlocks(3 * blockInterval)
                await twamm.connect(addr1).cancelTermSwapETHToToken(tokenB.address, 0, timeStamp+100000);
        ...
        describe("partial withdrawal", function () { 
            ...
            it("(ETH)proceeds can be withdrawn while order is still active(ETH)", async function () {
                ...
                const beforeBalanceB = await tokenB.balanceOf(addr2.address);
                await twamm.connect(addr2).instantSwapTokenToETH(tokenB.address, amountIn, timeStamp+100000);
